...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $newOrder = $this->order->whereStatus('Pending')->value('created_at');
        $productOrder = $this->productOrder->get();
        $user = $this->user->count();
        $total = $this->order->whereStatus('Completed')->value('total');

        return view('admin.dashboard', compact('category', 'product', 'user', 'order', 'date', 'newOrder', 'total'));
    }
}
