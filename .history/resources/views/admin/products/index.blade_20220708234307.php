...
<div class="card">
    ...
    <div class="card-body table-responsive">
      <table class="table table-hover">
        ...
        <tr>
            <td>{{$product->id}}</td>
            <a href="{{route('products.edit', $product->id)}}"><td><img src="{{$product->images->count() > 0 ?
                asset('/upload/imgs/' . $product->images->first()->url) : '/upload/imgs/default.jpg'}}" alt=""
                style="width: 50px; height:50px; border-radius:50%;"></td></a>
            <a href="{{route('products.show', $product->id)}}"><td>{{$product->name}}</td></a>
            <td>{{$product->price}}</td>
            <td>{{$product->sale}}</td>
            <td>
                <div class="row">
