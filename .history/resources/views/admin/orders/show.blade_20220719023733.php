                ...
                <form action="{{route('orders.update', $order->id)}}" method="post">
                    ...
                    <div class="col-3 text-center">
                        <select name="status" class="form-control h-100">
                            <option value="pending" {{$order->status == 0 ?? 'selected'}}>Pending</option>
                            <option value="confirm" {{$order->status == 1 ?? 'selected'}}>Confirm</option>
                            <option value="delivery" {{$order->status == 2 ?? 'selected'}}>Delivery</option>
                            <option value="success" {{$order->status == 3 ?? 'selected'}}>Success</option>
                        </select>
                    </div>
