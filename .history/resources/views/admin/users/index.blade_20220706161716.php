...
<div class="card">
    ...
    <div class="card-body table-responsive">
      <table class="table table-hover">
        ...
        <tr>
            <td>{{$user->id}}</td>
            <td><img src="{{$user->images->count()>0 ? asset('/upload/users/' . $user->images->first()->url) : '/upload/users/default.jpg'}}" alt="" style="width: 100px; height: 100px; border-radius:50%;"></td>
            <td>{{$user->name}}</td>
            <td>{{$user->email}}</td>
            <td>{{$user->phone}}</td>
            <td>
