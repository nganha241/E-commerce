            ...
            <div class="card-body p-5">
              ...
              <div class="mx-n5 px-5 py-4" style="background-color: #f2f2f2;">
                    ...
                    <div class="media product-card">
                        <a class="pull-left" href="{{route('single', $item->products->id)}}">
                        {{-- <img class="media-object" src="{{$item->product->image()}}" alt="Image" /> --}}
                        </a>
                            <div class="media-body">
                                <h4 class="media-heading"><a href="{{route('single', $item->products->id)}}">{{$item->products->name}}
                                </a></h4>
                                {{-- <p class="price">{{$item->product_quantity}} x {{$item->product->sale()}}</p> --}}
                                {{-- <input type="text" name="product_quantity" value="{{$item->product_quantity}}" hidden> --}}
                                <span class="remove" >Remove</span>
                            </div>
                    </div>
                    <div class="row">
                        <div class="col-md-8 col-lg-9">
                          <p class="mb-0">{{$item->products->name}}</p>
                        </div>
