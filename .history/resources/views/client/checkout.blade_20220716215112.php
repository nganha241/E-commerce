                ...
                <div class="product-checkout-details">
                    <div class="block">
                     ...
                     <ul class="summary-prices">
                        ...
                        <li>
                            <span>Coupon:</span>
                            <span>{{session('discount_amount_price')}}</span>
                        </li>
                        @endif
                     </ul>
                     <div class="summary-total">
                        <span>Total</span>
                        @if ($sum > 300000)
                                <span>{{number_format(round($sum, 3), 0, ',', ', ')}}</span>
                           @else
                                <span>{{number_format(round($sum + 30000), 0, ',', ', ')}}</span>
                           @endif
                     </div>
                  </div>
               </div>
         ...
         <div class="modal-content">
            <div class="modal-body">
               <form action="{{route('carts.applyCoupon')}}" method="POST">
                @csrf
                  <div class="form-group">
                     <input class="form-control" type="text" name="coupon_code" value="session('discount_amount_price')" placeholder="Enter Coupon Code">
                  </div>
